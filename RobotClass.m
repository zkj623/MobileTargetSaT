...
%             this.N = N;
        ...
        function [this,optz,list_tmp] = Planner(this,fld,sim,plan_mode,list_tmp,tt,ii)
            ...
            if ~is_tracking
                ...
                inputs1 = [30 pi/4]; %Constant speed and turning left
                [t1,p1] = ode45(@(t,y)derivative(kinematicModel,y,inputs1),tspan,initialState);
                a(:,1) = [p1(end,1);p1(end,2);p1(end,3)-pi/2;inputs1(1);1];

                inputs2 = [30 pi/8]; %Constant speed and turning left
                [t2,p2] = ode45(@(t,y)derivative(kinematicModel,y,inputs2),tspan,initialState);
                a(:,2) = [p2(end,1);p2(end,2);p2(end,3)-pi/2;inputs2(1);2];

                inputs3 = [30 -pi/4]; %Constant speed and turning left
                [t3,p3] = ode45(@(t,y)derivative(kinematicModel,y,inputs3),tspan,initialState);
                a(:,3) = [p3(end,1);p3(end,2);p3(end,3)-pi/2;inputs3(1);3];

                inputs4 = [30 -pi/8]; %Constant speed and turning left
                [t4,p4] = ode45(@(t,y)derivative(kinematicModel,y,inputs4),tspan,initialState);
                a(:,4) = [p4(end,1);p4(end,2);p4(end,3)-pi/2;inputs4(1);4];

                inputs5 = [30 0]; %Constant speed and turning left
                [t5,p5] = ode45(@(t,y)derivative(kinematicModel,y,inputs5),tspan,initialState);
                a(:,5) = [p5(end,1);p5(end,2);p5(end,3)-pi/2;inputs5(1);5];

                inputs6 = [0 pi/4]; %Constant speed and turning left
                [t6,p6] = ode45(@(t,y)derivative(kinematicModel,y,inputs6),tspan,initialState);
                a(:,6) = [p6(end,1);p6(end,2);p6(end,3)-pi/2;inputs6(1);6];

                inputs7 = [0 -pi/4]; %Constant speed and turning left
                [t7,p7] = ode45(@(t,y)derivative(kinematicModel,y,inputs7),tspan,initialState);
                a(:,7) = [p7(end,1);p7(end,2);p7(end,3)-pi/2;inputs7(1);7];
            else
                %
                a = zeros(5,20); % action space
                interpolated_points = zeros(20,2,3);

                inputs1 = [15 pi/16]; %Constant speed and turning left
                [t1,p1] = ode45(@(t,y)derivative(kinematicModel,y,inputs1),tspan,initialState);
                a(:,1) = [p1(end,1);p1(end,2);p1(end,3)-pi/2;inputs1(1);1];

                inputs2 = [10 pi/16]; %Constant speed and turning left
                [t2,p2] = ode45(@(t,y)derivative(kinematicModel,y,inputs2),tspan,initialState);
                a(:,2) = [p2(end,1);p2(end,2);p2(end,3)-pi/2;inputs2(1);2];

                inputs3 = [5 pi/16]; %Constant speed and turning left
                [t3,p3] = ode45(@(t,y)derivative(kinematicModel,y,inputs3),tspan,initialState);
                a(:,3) = [p3(end,1);p3(end,2);p3(end,3)-pi/2;inputs3(1);3];

                inputs4 = [15 pi/8]; %Constant speed and turning left
                [t4,p4] = ode45(@(t,y)derivative(kinematicModel,y,inputs4),tspan,initialState);
                a(:,4) = [p4(end,1);p4(end,2);p4(end,3)-pi/2;inputs4(1);4];

                inputs5 = [10 pi/8]; %Constant speed and turning left
                [t5,p5] = ode45(@(t,y)derivative(kinematicModel,y,inputs5),tspan,initialState);
                a(:,5) = [p5(end,1);p5(end,2);p5(end,3)-pi/2;inputs5(1);5];

                inputs6 = [5 pi/8]; %Constant speed and turning left
                [t6,p6] = ode45(@(t,y)derivative(kinematicModel,y,inputs6),tspan,initialState);
                a(:,6) = [p6(end,1);p6(end,2);p6(end,3)-pi/2;inputs6(1);6];

                inputs7 = [15 -pi/16]; %Constant speed and turning left
                [t7,p7] = ode45(@(t,y)derivative(kinematicModel,y,inputs7),tspan,initialState);
                a(:,7) = [p7(end,1);p7(end,2);p7(end,3)-pi/2;inputs7(1);7];

                inputs8 = [10 -pi/16]; %Constant speed and turning left
                [t8,p8] = ode45(@(t,y)derivative(kinematicModel,y,inputs8),tspan,initialState);
                a(:,8) = [p8(end,1);p8(end,2);p8(end,3)-pi/2;inputs8(1);8];

                inputs9 = [5 -pi/16]; %Constant speed and turning left
                [t9,p9] = ode45(@(t,y)derivative(kinematicModel,y,inputs9),tspan,initialState);
                a(:,9) = [p9(end,1);p9(end,2);p9(end,3)-pi/2;inputs9(1);9];

                inputs10 = [15 -pi/8]; %Constant speed and turning left
                [t10,p10] = ode45(@(t,y)derivative(kinematicModel,y,inputs10),tspan,initialState);
                a(:,10) = [p10(end,1);p10(end,2);p10(end,3)-pi/2;inputs10(1);10];

                inputs11 = [10 -pi/8]; %Constant speed and turning left
                [t11,p11] = ode45(@(t,y)derivative(kinematicModel,y,inputs11),tspan,initialState);
                a(:,11) = [p11(end,1);p11(end,2);p11(end,3)-pi/2;inputs11(1);11];

                inputs12 = [5 -pi/8]; %Constant speed and turning left
                [t12,p12] = ode45(@(t,y)derivative(kinematicModel,y,inputs12),tspan,initialState);
                a(:,12) = [p12(end,1);p12(end,2);p12(end,3)-pi/2;inputs12(1);12];

                inputs13 = [15 0]; %Constant speed and turning left
                [t13,p13] = ode45(@(t,y)derivative(kinematicModel,y,inputs13),tspan,initialState);
                a(:,13) = [p13(end,1);p13(end,2);p13(end,3)-pi/2;inputs13(1);13];

                inputs14 = [10 0]; %Constant speed and turning left
                [t14,p14] = ode45(@(t,y)derivative(kinematicModel,y,inputs14),tspan,initialState);
                a(:,14) = [p14(end,1);p14(end,2);p14(end,3)-pi/2;inputs14(1);14];

                inputs15 = [5 0]; %Constant speed and turning left
                [t15,p15] = ode45(@(t,y)derivative(kinematicModel,y,inputs15),tspan,initialState);
                a(:,15) = [p15(end,1);p15(end,2);p15(end,3)-pi/2;inputs15(1);15];

                inputs16 = [0 pi/16]; %Constant speed and turning left
                [t16,p16] = ode45(@(t,y)derivative(kinematicModel,y,inputs16),tspan,initialState);
                a(:,16) = [p16(end,1);p16(end,2);p16(end,3)-pi/2;inputs16(1);16];

                inputs17 = [0 -pi/16]; %Constant speed and turning left
                [t17,p17] = ode45(@(t,y)derivative(kinematicModel,y,inputs17),tspan,initialState);
                a(:,17) = [p17(end,1);p17(end,2);p17(end,3)-pi/2;inputs17(1);17];

                inputs18 = [0 pi/8]; %Constant speed and turning left
                [t18,p18] = ode45(@(t,y)derivative(kinematicModel,y,inputs18),tspan,initialState);
                a(:,18) = [p18(end,1);p18(end,2);p18(end,3)-pi/2;inputs18(1);18];

                inputs19 = [0 -pi/8]; %Constant speed and turning left
                [t19,p19] = ode45(@(t,y)derivative(kinematicModel,y,inputs19),tspan,initialState);
                a(:,19) = [p19(end,1);p19(end,2);p19(end,3)-pi/2;inputs19(1);19];

                inputs20 = [0 0]; %Constant speed and turning left
                [t20,p20] = ode45(@(t,y)derivative(kinematicModel,y,inputs20),tspan,initialState);
                a(:,20) = [p20(end,1);p20(end,2);p20(end,3)-pi/2;inputs20(1);20];
                %}
            end
            for mm = 1:size(interpolated_points,1)
                p = eval(strcat('p',num2str(mm)));
                for jj = 1:2
...
%    [B,w] = PF(node.state,B,w,o,r,zeros(2,2),R,I,h,list_obstacle);
    ...
    for ii = 1:size(B_tmp1,2)
        ...
        jj = jj+1;
                ...
                %     if ~is_tracking&&norm([B(1,:)*w';B(2,:)*w']-node.state(1:2))<50
                %         flag3 = region(ceil(B_tmp1(1,:)),ceil(B_tmp1(2,:)));
                %         flag3 = diag(flag3)';
